...
// =====================================================================
{
  ...
  // 3 фазы (v2: часть пунктов уже сделано в пакетах 1-5)
  const phases = [
    {
      tag: "✅ СДЕЛАНО в v2",
      color: C.green,
      items: [
        ...
        "+8 источников: AUD/CLP/CNY, Mining ETFs, VIX",
      ],
    },
    {
      tag: "БЛИЖАЙШИЕ 3-6 мес",
      color: C.copper,
      items: [
        "Накопить историю LME 3M (3-6 мес ежедневных запусков)",
        "FRED API: добавить Caixin PMI, IPI, ставки",
        "ERP-интеграция: привязка прогноза к плану закупок",
        "Прогноз цены в рублях (курс ЦБ + хедж)",
        "Telegram-алерты на смену режима и critical-новости",
      ],
    },
    {
      tag: "6 → 12 мес",
      color: C.navy,
      items: [
        "Платные источники: ICSG Bulletin, LMEsource, SMM",
        "Deep models: LSTM, Temporal Fusion Transformer",
        "Event study: автопривязка новостей к движениям",
        "FinBERT для NLP-сентимента (точнее VADER)",
        "Оптимизация портфеля: хедж + спот + контракты",
      ],
    },
  ];
