...
// =====================================================================
{
  ...
  s.addText("Доклад для руководства · 7 минут чтения · 16 слайдов", {
...
// =====================================================================
{
  ...
  const blocks = [
    ...
    { num: "2", title: "Какие показатели используем",
      desc: "7 групп факторов, веса каждого, почему именно они" },
    { num: "3", title: "Как считается прогноз",
      desc: "4 «головы» модели — каждая считает по-своему" },
    { num: "4", title: "Где модель сильная, где слабая",
      desc: "Что система видит и чего точно не видит" },
    { num: "5", title: "Зачем нужен эксперт по меди",
      desc: "Что человек делает, чего модель не сможет никогда" },
    { num: "6", title: "Что улучшать дальше",
      desc: "План развития на 6-12 месяцев" },
  ];
...
// =====================================================================
{
  ...
  header(s, "Какие показатели мы используем",
            "7 групп. Каждая отвечает за свой класс сигналов");

  // 7 карточек в сетке 4+3
  const groups = [
    { tag: "①", name: "История цены меди",     why: "Тренд, сезонность",
      weight: "40 %",  c: C.copper },
    { tag: "②", name: "Доллар (DXY)",          why: "Обратная связь с медью",
      weight: "12 %",  c: C.navy },
    { tag: "③", name: "Связанные товары",       why: "Нефть, золото, серебро",
      weight: "10 %",  c: C.navy },
    { tag: "④", name: "Позиции спекулянтов (COT)", why: "Опережающий сигнал",
      weight: "15 %",  c: C.copper },
    { tag: "⑤", name: "Запасы LME",             why: "Физический баланс",
      weight: "8 %",   c: C.navy },
    { tag: "⑥", name: "Технические индикаторы", why: "Состояние тренда",
      weight: "10 %",  c: C.copper },
    { tag: "⑦", name: "Календарь и режим рынка", why: "Контекст времени",
      weight: "5 %",   c: C.navy },
  ];

  groups.forEach((g, i) => {
    const col = i % 4, row = Math.floor(i / 4);
    const x = 0.4 + col * 2.4, y = 1.4 + row * 1.6;
    const w = 2.25, h = 1.4;
    s.addShape(pres.shapes.RECTANGLE, {
      x, y, w, h,
      fill: { color: C.cream }, line: { color: C.ice, width: 1 },
...
// =====================================================================
{
  ...
  bullet(s, 5.85, 1.85, 4, 3.0, [
    "Более половины (55 %) — цена и спекулянты. Это «душа» рынка.",
    "Макро (доллар + связанные товары + ставки) — четверть веса.",
    "Технические и календарные — около 15 %.",
    "Физические запасы пока 8 % — будет расти при накоплении истории.",
  ], 12);

  s.addText("Веса корректируются автоматически каждые 20 бизнес-дней при переобучении.",
...
// =====================================================================
{
  ...
  // Снизу — финальная фраза
  s.addText(
    "Итоговый прогноз = средневзвешенное всех 4 моделей в логарифмическом пространстве. Коридор p10-p90 — естественная неопределённость, проверенная на 5 годах истории.",
...
// =====================================================================
//  Слайд 13 — Что система видит / не видит
// =====================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
...
// =====================================================================
//  Слайд 14 — Зачем нужен эксперт по меди (главный слайд)
// =====================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
...
// =====================================================================
//  Слайд 15 — Что улучшать
// =====================================================================
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  header(s, "Что улучшать дальше",
  ...
  const phases = [
    {
      ...
      color: C.green,
      items: [
        "Накопить историю запасов LME (3 мес = полезная история)",
        "Добавить курс юаня и Caixin PMI через FRED API",
        "Адаптивные веса моделей по текущему режиму",
        "NLP-сентимент свежих новостей (положительный/отрицательный тон)",
      ],
    },
    {
      tag: "3 → 6 мес",
      color: C.copper,
      items: [
        "Интеграция с ERP — автоматическая привязка прогноза к плану закупок",
        "Прогноз цены в рублях через интеграцию с курсом ЦБ",
        "Учёт условий долгосрочных контрактов (формулы, базы, премии)",
        "Алерты в Telegram/Email при изменении режима или critical-новостях",
      ],
    },
    {
      tag: "6 → 12 мес",
      color: C.navy,
      items: [
        "Платные источники: ICSG Monthly Bulletin, LME 3M, SMM Yangshan",
        "Глубокие модели (LSTM, Temporal Fusion Transformer)",
        "Event study — автоматическая привязка новостей к ценовым движениям",
        "Оптимизация портфеля закупок (хедж + спот + контракты)",
      ],
    },
  ];
  ...
  s.addText("Бюджет: первая фаза — без затрат (опираемся на бесплатные данные). Платные источники — со второй половины года.",
...
// =====================================================================
{
  ...
  s.addText([
    { text: "Прогноз на 5 горизонтов + коридоры",  options: { breakLine: true, color: C.charcoal } },
    { text: "7 групп показателей · 4 модели",      options: { breakLine: true, color: C.charcoal } },
    { text: "Время-слайдер истории · новости · события", options: { color: C.charcoal } },
  ], {
    x: 0.7, y: 3.5, w: 4.0, h: 1.0,
    fontSize: 11, fontFace: FONT, margin: 0, valign: "top",
  });
  ...
  s.addText([
    ...
    { text: "Решение по платным источникам в Q3 2026", options: { color: C.charcoal } },
  ], {
